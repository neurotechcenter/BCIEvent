...
void BCIEvent_N::BCIEventApplication::InitBCIEvent(){
	...
	button->self()
		...
		.addEventListener(std::make_unique<EventListener>(ProtoSequence()
			...
			.addNormal([&](Sequence& callingSequence) {callingSequence.actorMoveTo(0.5, 0.5);  })
			.addNormal([&](Sequence& callingSequence) {callingSequence.actorSay("wait");  })
			.addTimer(2)
		...
		.addEventListener("go", std::make_unique<EventListener>(ProtoSequence()
			...
			.addNormal([&](Sequence& callingSequence) {callingSequence.actorSay("click");  })
		...
		.addEventListener("click", std::make_unique<EventListener>(ProtoSequence()
			.addIf([&](SequenceEnvironment& callingSequence) {return std::get<bool>(callingSequence.getVariable("ready")); })
			...
			.addIfElse([&](SequenceEnvironment& callingSequence) { return std::get<bool>(callingSequence.getVariable("late")); })
			.addNormal([&](Sequence& callingSequence) {callingSequence.actorSay("You took more than 1 second");  })
			.closeStatement()
			.addNormal([&](Sequence& callingSequence) {callingSequence.actorSay("You took " + std::to_string(callingSequence.getTimer("reactionTime").time()) + " seconds.");  })
			.closeStatement()
			.closeStatement()
			.addTimer(3)
			.addNormal([&](Sequence& callingSequence) {callingSequence.callEvent("go"); })
